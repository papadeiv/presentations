...
time = readin("../data/slide_03/time.csv")
μ = readin("../data/slide_03/μ.csv")
x1 = readin("../data/slide_03/x1.csv")
x2 = readin("../data/slide_03/x2.csv")
...
@showprogress for n in 1:length(μ)
        ...
        lines!(ax, x1[1:n], x2[1:n], color = :black, linewidth = 3)
        # Plot the equilibria
        if μ[n] <= 0.81061
        ...
        # Plot the IC
        scatter!(ax, x1[1], x2[1], color = :yellow, markersize = 30, strokewidth = 3)
 
        # Export the phase plane figure 
        save("../fig/slide_03/phase_portrait/$n.png", fig)
        ...
        # Plot the trajectories
        lines!(ax, time[1:n], x1[1:n], color = :black, linewidth = 3)
        # Plot the equilibria
        if μ[n] <= 0.81061
        ...
        # Plot the IC
        scatter!(ax, 0.0, x1[1], color = :yellow, markersize = 30, strokewidth = 3)

        # Export the timeseries figure 
        save("../fig/slide_03/timeseries/$n.png", fig)
end
