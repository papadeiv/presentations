...
t = readin("../data/slide_09/time.csv")
μ = readin("../data/slide_09/μ.csv")
u = readin("../data/slide_09/u.csv")

# Define arrays to store the timeseries until the N-tipping event
ut = Float64[]
μt = Float64[]
...
# Detect the N-tipping and extract the subset timeseries prior to that
for n in 1:length(u)
        push!(ut, u[n])
        push!(μt, μ[n])
        if u[n] < -0.2  
...
# Export the data
writeout(hcat(μt, ut), "../data/slide_09/ut.csv")
writeout(hcat(μ1, x1), "../data/slide_09/stable_eq_1.csv")
writeout(hcat(μ2, x2), "../data/slide_09/stable_eq_2.csv")
writeout(hcat(μ3, x3), "../data/slide_09/unstable_eq.csv")
