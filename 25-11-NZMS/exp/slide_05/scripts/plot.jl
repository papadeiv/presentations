...
function plot(time, state, termination)
        ...
        if abs(state[end] - termination) < 0.01 
                # Plot the timeseries
                lines!(ax1, time, state, color = (:black, 0.5), linewidth = 1.0)
        end
end
...
function plot(termination::Float64, total_time::Float64)
        ...
        lines!(ax1, [0,T], [Y, Y], color = :black, linewidth = 6.0, linestyle = :dash)

        # Plot the minimiser of the action
        lines!(ax2, [0,T], [Y^2, Y^2], color = :black, linewidth = 6.0, linestyle = :dash)

        # Plot the full paths 
        lines!(ax1, time, [ɸ1(t) for t in time], color = CtpRed, linewidth = 8.0)
        lines!(ax1, time, [ɸ2(t) for t in time], color = CtpBlue, linewidth = 8.0)
